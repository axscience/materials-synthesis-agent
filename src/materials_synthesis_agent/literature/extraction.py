"""Citation-grounded protocol extraction via Claude's tool-use (forced structured output).
...
# changes. This is an ESTIMATE for the pre-call cost display, not a source of billing truth.
...
def estimate_extraction_cost(target: Target, paper: Paper, model: str = DEFAULT_MODEL) -> float:
    ...
    pricing = _PRICING_PER_MTOK_USD.get(model, _DEFAULT_PRICING)
...
def extract_protocol(
    ...
    a fake client that never hits the network -- see tests/test_extraction.py."""
    if dry_run:
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": _build_prompt(target, paper)}],
    )
